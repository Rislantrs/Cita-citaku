...
export default function BrandMark({ className = '', showLabel = true, showTagline = false, size = 'md' }: BrandMarkProps) {
  ...
  return (
    <div className={`inline-flex items-center gap-3 ${className}`}>
      <div className={`relative overflow-hidden ${sizeClasses[size]} border border-blue-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm`}>
        <img src={logoSrc} alt={t('app_name')} className="h-full w-full object-contain p-1" />
      </div>
      {showLabel && (
        <div className="leading-tight">
          <p className={`font-black tracking-tight text-slate-950 dark:text-white ${textSizeClasses[size]}`}>{t('app_name')}</p>
